...
class _QuizTimerState extends State<QuizTimer> {
  ...
  /// 🕒 Starts the countdown timer
  void _startTimer() {
    _timer?.cancel(); // Prevent overlapping timers
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (remainingSeconds > 0) {
        setState(() => remainingSeconds--);
      } else {
  ...
  /// 🔁 Public method to restart the timer with full duration
  void resetTimer() {
    _timer?.cancel();
    setState(() {
      remainingSeconds = widget.totalSeconds;
  ...
  /// 🚀 Public method to restart and start the timer
  void restartTimer() {
    resetTimer();
    _startTimer();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        ...
        const SizedBox(width: 6),
        Text(
          '${_formatTime(remainingSeconds)} s',
          style: GoogleFonts.lato(
            fontSize: 20,
            fontWeight: FontWeight.bold,
            color: textColor,
